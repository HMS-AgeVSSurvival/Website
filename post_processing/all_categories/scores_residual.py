                ...
                for score in SCORES[target]:
                    ...
                    if score != "diff_c_index":
                        ...
                        for fold in FOLDS:
                            ...
                            new_scores.loc[(idx_best_1, algorithm), (target, fold, score)] = (
                                new_scores.loc[(idx_best_1, algorithm), (target, fold, "c_index")].values
                                - results_1.loc[
                                    idx_best_1, (f"Basic survival {target}", algorithm, f"{fold} C-index")
                                ].values
                            )

                            new_scores.loc[(idx_best_0, algorithm), (target, fold, f"{score}_std")] = np.sqrt(
                                new_scores.loc[(idx_best_0, algorithm), (target, fold, "c_index_std")].values ** 2
                                + results_0.loc[
                            ...
                            new_scores.loc[(idx_best_1, algorithm), (target, fold, f"{score}_std")] = np.sqrt(
                                new_scores.loc[(idx_best_1, algorithm), (target, fold, "c_index_std")].values ** 2
                                + results_1.loc[
