    ...
    for method in tqdm(METHODS):
        ...
        for main_category in MAIN_CATEGORIES:
            ...
            new_correlations_between_targets = pd.DataFrame(
                index=pd.MultiIndex.from_product((correlations.index.to_list(), ALGORITHMS)),
                columns=pd.MultiIndex.from_product((TARGETS_TARGETS, ["correlation", "std"])),
            )
            new_correlations_between_algorithms = pd.DataFrame(
                index=pd.MultiIndex.from_product((correlations.index.to_list(), TARGETS)),
                ...
                for algorithm in ALGORITHMS:
                    if target_a == target_b:
                        new_correlations_between_targets.loc[
                            (slice(None), algorithm), (target_vs_target, "correlation")
                        ] = correlations[("same_target_same_algorithm", target_a, algorithm)]
                        new_correlations_between_targets.loc[
                            (slice(None), algorithm), (target_vs_target, "std")
                        ] = correlations_std[("same_target_same_algorithm", target_a, algorithm)]
                    else:
                        new_correlations_between_targets.loc[
                            (slice(None), algorithm), (target_vs_target, "correlation")
                        ] = correlations[("same_algorithm", target_vs_target, algorithm)]
                        new_correlations_between_targets.loc[
                            (slice(None), algorithm), (target_vs_target, "std")
                        ] = correlations_std[("same_algorithm", target_vs_target, algorithm)]
                ...
                for target in TARGETS:
                    if algorithm_a == algorithm_b:
                        new_correlations_between_algorithms.loc[
                            (slice(None), target), (algorithm_vs_algorithm, "correlation")
                        ] = correlations[("same_target_same_algorithm", target, algorithm_a)]
                        new_correlations_between_algorithms.loc[
                            (slice(None), target), (algorithm_vs_algorithm, "std")
                        ] = correlations_std[("same_target_same_algorithm", target, algorithm_a)]
                    else:
                        new_correlations_between_algorithms.loc[
                            (slice(None), target), (algorithm_vs_algorithm, "correlation")
                        ] = correlations[("same_target", target, algorithm_vs_algorithm)]
                        new_correlations_between_algorithms.loc[
                            (slice(None), target), (algorithm_vs_algorithm, "std")
                        ] = correlations_std[("same_target", target, algorithm_vs_algorithm)]
