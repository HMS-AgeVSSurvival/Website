...
@APP.callback(
    Output("memory_prediction_performances", "data"),
    [Input("method_prediction_performances", "value")]
    + [Input(f"target_{axis}_prediction_performances", "value") for axis in AXES],
)
def get_prediction_performances(method, target_row, target_column):
...
@APP.callback(
    Output("memory_std_prediction_performances", "data"),
    [Input("method_prediction_performances", "value")]
    + [Input(f"target_{axis}_prediction_performances", "value") for axis in AXES],
)
def get_prediction_performances_std(method, target_row, target_column):
...
@APP.callback(
    Output("memory_number_participants_prediction_performances", "data"),
    [Input(f"target_{axis}_prediction_performances", "value") for axis in AXES],
)
def get_prediction_performances_number_partitipants(target_row, target_column):
...
def get_controls_prediction_performances():
    return [
        get_item_radio_items("method_prediction_performances", METHODS, "Method:"),
    ]


def get_controls_axis_prediction_performances(key_axis):
    return (
        [get_item_radio_items(f"target_{key_axis}_prediction_performances", TARGETS, "Target:")]
        + [
            get_drop_down(
                f"{main_category}_category_{key_axis}_prediction_performances",
        ...
        + [get_check_list(f"algorithm_{key_axis}_prediction_performances", ALGORITHMS, "Algorithm:")]
    )
...
for main_category in MAIN_CATEGORIES:
    for axis in AXES:
        ...
        @APP.callback(
            Output(f"{main_category}_category_{axis}_prediction_performances", "value"),
            Input(f"{main_category}_category_{axis}_prediction_performances", "value"),
        )
...
@APP.callback(
    [Output("title_prediction_performances", "children"), Output("heatmap_prediction_performances", "figure")],
    [
        Input("memory_prediction_performances", "data"),
        Input("memory_std_prediction_performances", "data"),
        Input("memory_number_participants_prediction_performances", "data"),
    ]
    + [
        Input(f"{main_category}_category_{axis}_prediction_performances", "value")
        for main_category in MAIN_CATEGORIES
    ...
    + [Input(f"algorithm_{axis}_prediction_performances", "value") for axis in AXES],
)
...
LAYOUT = dbc.Container(
    [
        dcc.Loading(
            [
                dcc.Store(id="memory_prediction_performances"),
                dcc.Store(id="memory_std_prediction_performances"),
                dcc.Store(id="memory_number_participants_prediction_performances"),
            ]
        ),
        html.H1("Residual correlations"),
        html.Br(),
        html.Br(),
        dbc.Row(dbc.Col(dbc.Card(get_controls_prediction_performances()), width={"size": 2, "offset": 5})),
        html.Br(),
        html.Br(),
        dbc.Row(
            [
        ...
        dbc.Row(
            [
                dbc.Col(dbc.Card(get_controls_axis_prediction_performances(key_axis)), width={"size": 6})
        ...
        dbc.Col(
            [
                dcc.Loading(
                    [
                        html.H3(id="title_prediction_performances"),
                        dcc.Graph(id="heatmap_prediction_performances", config=DOWNLOAD_CONFIG),
                    ]
                )
            ],
            width={"offset": 2},
