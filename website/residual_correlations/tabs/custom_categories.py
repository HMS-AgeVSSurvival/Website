...
@APP.callback(
    Output("memory_residual_correlations_custom_categories", "data"),
    [Input("method_residual_correlations_custom_categories", "value")]
    + [Input(f"target_{key_axis}_residual_correlations_custom_categories", "value") for key_axis in AXES],
)
def get_residual_correlations_custom_categories(method, target_row, target_column):
    return load_correlations(method, target_row, target_column, std_path="")


@APP.callback(
    Output("memory_std_residual_correlations_custom_categories", "data"),
    [Input("method_residual_correlations_custom_categories", "value")]
    + [Input(f"target_{key_axis}_residual_correlations_custom_categories", "value") for key_axis in AXES],
)
def get_std_residual_correlations_custom_categories(method, target_row, target_column):
...
def load_correlations(method, target_row, target_column, std_path=""):
    path_to_fetch = f"custom_categories/correlations/residual/{method}{std_path}_{target_row}_{target_column}.feather"
    if does_key_exists(path_to_fetch):
        return load_feather(path_to_fetch).to_dict()
    else:
        correlations = load_feather(
            f"custom_categories/correlations/residual/{method}{std_path}_{target_column}_{target_row}.feather"
...
@APP.callback(
    Output("memory_number_participants_residual_correlations_custom_categories", "data"),
    [Input(f"target_{key_axis}_residual_correlations_custom_categories", "value") for key_axis in AXES],
)
def get_number_partitipants_residual_correlations_custom_categories(target_row, target_column):
    path_to_fetch = f"custom_categories/correlations/residual/number_participants_{target_row}_{target_column}.feather"
    if does_key_exists(path_to_fetch):
        return load_feather(path_to_fetch).to_dict()
    else:
        number_participants = load_feather(
            f"custom_categories/correlations/residual/number_participants_{target_column}_{target_row}.feather"
...
def get_controls_residual_correlations_custom_categories():
    return [
        get_item_radio_items("method_residual_correlations_custom_categories", METHODS, "Method:"),
    ]


def get_controls_axis_residual_correlations_custom_categories(key_axis):
    return (
        [get_item_radio_items(f"target_{key_axis}_residual_correlations_custom_categories", TARGETS, "Target:")]
        + [
            get_drop_down(
                f"{main_category}_category_{key_axis}_residual_correlations_custom_categories",
        ...
        + [get_check_list(f"algorithm_{key_axis}_residual_correlations_custom_categories", ALGORITHMS, "Algorithm:")]
    )
...
for main_category in MAIN_CATEGORIES:
    for key_axis in AXES:
        ...
        @APP.callback(
            Output(f"{main_category}_category_{key_axis}_residual_correlations_custom_categories", "value"),
            Input(f"{main_category}_category_{key_axis}_residual_correlations_custom_categories", "value"),
        )
        def _update_categories_row_residual_correlations_custom_categories(categories):
...
for key_axis in AXES:
    ...
    @APP.callback(
        Output(f"algorithm_{key_axis}_residual_correlations_custom_categories", "value"),
        Input(f"algorithm_{key_axis}_residual_correlations_custom_categories", "value"),
    )
...
@APP.callback(
    [
        Output("title_residual_correlations_custom_categories", "children"),
        Output("heatmap_residual_correlations_custom_categories", "figure"),
    ],
    [
        Input("memory_residual_correlations_custom_categories", "data"),
        Input("memory_std_residual_correlations_custom_categories", "data"),
...
def get_custom_categories():
    return dbc.Container(
        [
            dcc.Loading(
                [
                    dcc.Store(id="memory_residual_correlations_custom_categories"),
                    dcc.Store(id="memory_std_residual_correlations_custom_categories"),
            ...
            dbc.Row(
                dbc.Col(
                    dbc.Card(get_controls_residual_correlations_custom_categories()), width={"size": 2, "offset": 5}
                )
            ),
            html.Br(),
            html.Br(),
            ...
            dbc.Row(
                [
                    dbc.Col(
                        dbc.Card(get_controls_axis_residual_correlations_custom_categories(key_axis)), width={"size": 6}
                    )
                    for key_axis in AXES
                ]
            ),
            dbc.Col(
                [
                    dcc.Loading(
                        [
                            html.H3(id="title_residual_correlations_custom_categories"),
                            dcc.Graph(id="heatmap_residual_correlations_custom_categories", config=DOWNLOAD_CONFIG),
                        ]
                    )
                ],
                width={"offset": 2},
