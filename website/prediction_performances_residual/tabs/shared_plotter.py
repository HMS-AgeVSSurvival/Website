...
def plot_scores(
    ...
    if algorithms == ["best"]:
        ...
        for target in targets:
            ...
            best_algorithms = pd.Series(best_algorithms).replace(np.nan, algorithms_to_look_at[0])
    ...
    for fold in FOLDS_RESIDUAL:
        ...
        if scores_fold.shape[0] > 1:
            titles[
                fold
            ] = f"{FOLDS_RESIDUAL[fold]}, average {SCORES_RESIDUAL[targets[0]][metric]} = {pd.Series(scores_fold.loc[:, (targets, fold, metric)].values.flatten()).mean().round(3)} +- {pd.Series(scores_fold.loc[:, (targets, fold, metric)].values.flatten()).std().round(3)}"
        else:
