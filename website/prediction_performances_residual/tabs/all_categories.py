...
from website import TARGETS, MAIN_CATEGORIES, CATEGORIES, ALGORITHMS, FOLDS_RESIDUAL, SCORES, DOWNLOAD_CONFIG
    ...
    return (
        ...
        + [
            ...
            get_item_radio_items(
                "metric_prediction_performances_residual_all_categories", SCORES[list(TARGETS.keys())[0]], "Target:"
            ),
        ]
    )
...
for main_category in MAIN_CATEGORIES:
    ...
    def update_categories_prediction_performances_residual_all_categories(categories):
...
def update_targets_prediction_performances_residual_all_categories(targets):
...
def _fill_bars_prediction_performances_residual_all_categories(
    ...
    rename(scores, columns=False, custom_categories=False)
    rename(information, algorithm=False, columns=False, custom_categories=False)
            ...
            dcc.Loading(
                [
                    dcc.Store(
                        ...
                        data=pd.read_feather(f"data/all_categories/scores.feather").to_dict(),
                    ),
                    dcc.Store(
